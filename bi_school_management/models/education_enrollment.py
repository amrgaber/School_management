...
class EducationEnrollment(models.Model):
    ...
    course_id = fields.Many2one('education.course', string='Course', required=True, tracking=True)
    ...
    amount_total = fields.Monetary(string='Total Amount', related='invoice_id.amount_total', readonly=True)
    amount_due = fields.Monetary(string='Amount Due', related='invoice_id.amount_residual', readonly=True)
    ...
    total_classes = fields.Integer(string='Total Classes', compute='_compute_attendance_stats', store=True)
    attended_classes = fields.Integer(string='Attended Classes', compute='_compute_attendance_stats', store=True)
    ...
    # SQL Constraints
    _sql_constraints = [
        ('unique_enrollment', 'unique(student_id, course_id)',
         'Student can only be enrolled in a course once!'),
        ('valid_score', 'check(score >= 0 AND score <= 100)',
    ...
    def _compute_attendance_stats(self):
        for enrollment in self:
            if enrollment.attendance_ids:
                ...
                enrollment.attended_classes = len(enrollment.attendance_ids.filtered(lambda a: a.state == 'present'))
                enrollment.attendance_percentage = (enrollment.attended_classes / enrollment.total_classes) * 100 if enrollment.total_classes > 0 else 0.0
            else:
    ...
    def action_confirm(self):
        ...
        for enrollment in self:
            ...
            if self.env.context.get('notify_teacher') and enrollment.course_id.teacher_id:
    ...
    def action_complete(self):
        ...
        for enrollment in self:
            ...
            if enrollment.attendance_percentage < min_attendance:
                raise UserError(_('Student does not meet minimum attendance requirement (%d%%).') % min_attendance)
    ...
    def _check_prerequisites(self):
        ...
        for enrollment in self:
            if enrollment.course_id.prerequisite_ids:
                ...
                missing_prerequisites = enrollment.course_id.prerequisite_ids - completed_courses
    ...
    def _check_course_capacity(self):
        ...
        for enrollment in self:
            if hasattr(enrollment.course_id, 'capacity') and enrollment.course_id.capacity:
                current_enrollments = len(enrollment.course_id.enrollment_ids.filtered(
                    lambda e: e.state in ['confirmed', 'enrolled']
                ))
                if current_enrollments >= enrollment.course_id.capacity:
                    raise ValidationError(_('Course capacity exceeded. Cannot enroll more students.'))
    ...
    def _prepare_invoice_vals(self):
        ...
        return {
            ...
            'invoice_line_ids': [(0, 0, {
                'product_id': self.course_id.product_id.id if self.course_id.product_id else False,
    ...
    def _notify_teacher_enrollment(self):
        ...
        self.activity_schedule(
            ...
            note=_('New student %s enrolled in course %s.') % (self.student_id.name, self.course_id.name)
        )
