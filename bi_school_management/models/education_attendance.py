...
class EducationAttendance(models.Model):
    ...
    class_id = fields.Many2one('education.class', string='Class', required=True, tracking=True)
    enrollment_id = fields.Many2one('education.enrollment', string='Enrollment', tracking=True)
    date = fields.Date(string='Attendance Date', default=fields.Date.context_today, required=True, tracking=True)
    ...
    ], string='State', default='present', tracking=True)
    ...
    course_id = fields.Many2one('education.course', string='Course',
                               related='enrollment_id.course_id', store=True)
