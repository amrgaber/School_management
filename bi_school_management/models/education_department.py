...
class EducationDepartment(models.Model):
    ...
    school_id = fields.Many2one('education.school', string='School', required=True, tracking=True)
    head_teacher_id = fields.Many2one('hr.employee', string='Head Teacher', tracking=True)
    ...
    student_ids = fields.One2many('education.student', 'department_id', string='Students')

    # Computed Fields
    total_courses = fields.Integer(string='Total Courses', compute='_compute_totals', store=True)
    total_classes = fields.Integer(string='Total Classes', compute='_compute_totals', store=True)
