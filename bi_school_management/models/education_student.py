...
class EducationStudent(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Student", required=True, tracking=True
    )
    name = fields.Char(
        string="Student Name", related="partner_id.name", store=True, readonly=True
    )
    student_id = fields.Char(
        string="Student ID", required=True, copy=False, tracking=True
    )
    ...
    enrollment_ids = fields.One2many(
        "education.enrollment", "student_id", string="Course Enrollments"
    )
    attendance_ids = fields.One2many(
        "education.attendance", "student_id", string="Attendance Records"
    )

    # Computed Fields for Statistics
    total_enrollments = fields.Integer(
        string="Total Enrollments", compute="_compute_enrollments", store=True
    )
    active_enrollments = fields.Integer(
        string="Active Enrollments", compute="_compute_enrollments", store=True
    )
    attendance_percentage = fields.Float(
        string="Attendance %", compute="_compute_attendance_percentage", store=True
    )
    total_fees = fields.Monetary(
        string="Total Fees", compute="_compute_total_fees", store=True
    )
    outstanding_fees = fields.Monetary(
        string="Outstanding Fees", compute="_compute_total_fees", store=True
    )
    ...
    date_of_birth = fields.Date(
        string="Date of Birth", related="partner_id.date_of_birth", store=True
    )
    ...
    parent_enrollment_note = fields.Char(
        string="Enrollment Note", help="Note to be passed to new enrollments"
    )
    ...
    school_id = fields.Many2one(
        "education.school",
        string="School",
        related="department_id.school_id",
        store=True,
    )
    ...
    def _compute_is_class_teacher(self):
        ...
        for student in self:
            ...
            student.is_class_teacher = (
                class_teacher
                and class_teacher.user_id.id == self.env.context.get("uid")
            )
    ...
    def _compute_enrollments(self):
        for student in self:
            ...
            student.active_enrollments = len(
                student.enrollment_ids.filtered(lambda e: e.state == "enrolled")
            )
    ...
    def _compute_attendance_percentage(self):
        for student in self:
            if student.attendance_ids:
                ...
                present_days = len(
                    student.attendance_ids.filtered(lambda a: a.state == "present")
                )
    ...
    @api.depends(
        "enrollment_ids",
        "enrollment_ids.invoice_id",
        "enrollment_ids.invoice_id.payment_state",
    )
    def _compute_total_fees(self):
        for student in self:
            invoices = student.enrollment_ids.mapped("invoice_id").filtered(
                lambda inv: inv.move_type == "out_invoice"
            )
            student.total_fees = sum(invoices.mapped("amount_total"))
            student.outstanding_fees = sum(
    ...
    def action_enroll(self):
        ...
        self.write(
            {"state": "enrolled", "enrollment_date": fields.Date.context_today(self)}
        )
    ...
    def action_transfer(self):
        ...
        if self.state not in ["enrolled", "suspended"]:
            raise UserError(
                _("Only enrolled or suspended students can be transferred.")
            )
    ...
    def action_graduate(self):
        ...
        self.write(
            {"state": "graduated", "graduation_date": fields.Date.context_today(self)}
        )
    ...
    def action_dropout(self):
        ...
        if self.state not in ["enrolled", "suspended"]:
            raise UserError(
                _("Only enrolled or suspended students can be marked as dropped out.")
            )
    ...
    def _validate_enrollment(self):
        ...
        if (
            self.class_id.capacity
            and len(self.class_id.student_ids) >= self.class_id.capacity
        ):
            raise ValidationError(
                _("Class capacity exceeded. Cannot enroll more students.")
            )
        ...
        if self.age and self.age < min_age:
            raise ValidationError(
                _("Student is too young for enrollment. Minimum age: %d") % min_age
            )
    ...
    def _check_graduation_requirements(self):
        ...
        completed_courses = self.enrollment_ids.filtered(
    ...
    def action_view_enrollments(self):
        ...
        self.ensure_one()
        action = self.env.ref(
            "bi_school_management.action_education_enrollment"
        ).read()[0]
        action.update(
            {
    ...
    def action_view_invoice(self):
        ...
        self.ensure_one()
        invoice_ids = (
            self.enrollment_ids.mapped("invoice_id")
    ...
    def action_view_attendance(self):
        ...
        date_from = self.env.context.get(
            "date_from", fields.Date.context_today(self) - timedelta(days=30)
        )
