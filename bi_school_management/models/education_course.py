...
class EducationCourse(models.Model):
    ...
    fee_amount = fields.Monetary(string='Course Fee', currency_field='company_currency_id', tracking=True)
    company_currency_id = fields.Many2one('res.currency', string='Currency', related='department_id.school_id.company_id.currency_id', readonly=True)

    # Course Information
    description = fields.Text(string='Description')
    duration_hours = fields.Float(string='Duration (Hours)')
    credits = fields.Integer(string='Credits')
    capacity = fields.Integer(string='Maximum Students', help="Maximum number of students that can enroll in this course")
    required = fields.Boolean(string='Required Course', default=False,
                             help="Check if this course is required for graduation")

    # Prerequisites and Requirements
    prerequisite_ids = fields.Many2many('education.course', 'course_prerequisite_rel',
    ...
    # Computed Fields
    total_enrollments = fields.Integer(string='Total Enrollments', compute='_compute_totals', store=True)
    active_enrollments = fields.Integer(string='Active Enrollments', compute='_compute_totals', store=True)
    ...
    def _compute_totals(self):
        for course in self:
            ...
            course.active_enrollments = len(course.enrollment_ids.filtered(lambda e: e.state == 'enrolled'))
            if course.capacity:
