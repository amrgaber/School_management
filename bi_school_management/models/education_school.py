...
class EducationSchool(models.Model):
    ...
    department_ids = fields.One2many(
        "education.department", "school_id", string="Departments"
    )
    academic_year_ids = fields.One2many(
        "education.academic.year", "school_id", string="Academic Years"
    )
    ...
    total_departments = fields.Integer(
        string="Total Departments", compute="_compute_totals", store=True
    )
