...
class BulkAttendanceWizard(models.TransientModel):
    ...
    department_id = fields.Many2one('education.department', string='Department',
                                   related='class_id.department_id', readonly=True)
    academic_year_id = fields.Many2one('education.academic.year', string='Academic Year',
                                      related='class_id.academic_year_id', readonly=True)
    ...
    attendance_line_ids = fields.One2many('education.bulk.attendance.line', 'wizard_id', string='Attendance Lines')
    ...
    ], string='Bulk Action', default='custom')

    # Summary Information (Step 4)
    total_students = fields.Integer(string='Total Students', compute='_compute_summary', store=False)
    present_count = fields.Integer(string='Present Count', compute='_compute_summary', store=False)
    absent_count = fields.Integer(string='Absent Count', compute='_compute_summary', store=False)
    late_count = fields.Integer(string='Late Count', compute='_compute_summary', store=False)

    # Additional Options
    send_notifications = fields.Boolean(string='Send Notifications to Parents', default=False)
    create_activities = fields.Boolean(string='Create Activities for Absences', default=False)
    ...
    def _compute_summary(self):
        for wizard in self:
            ...
            wizard.present_count = len(wizard.attendance_line_ids.filtered(lambda l: l.state == 'present'))
            wizard.absent_count = len(wizard.attendance_line_ids.filtered(lambda l: l.state == 'absent'))
            wizard.late_count = len(wizard.attendance_line_ids.filtered(lambda l: l.state == 'late'))
    ...
    def _onchange_class_id(self):
        ...
        if self.class_id:
            ...
            if self.env.context.get('only_active_enrollments'):
                active_students = self.env['education.enrollment'].search([
    ...
    def _onchange_course_id(self):
        ...
        if self.course_id:
            enrolled_students = self.env['education.enrollment'].search([
    ...
    def _onchange_all_students(self):
        ...
        if self.all_students and self.class_id:
            ...
            if self.env.context.get('enrolled_only'):
                students = self.class_id.student_ids.filtered(lambda s: s.state == 'enrolled')
            else:
    ...
    def action_next_step(self):
        ...
        if self.step == 'selection':
            ...
            if not self.env.context.get('allow_duplicate_attendance'):
                ...
                if existing:
                    raise UserError(_('Attendance already exists for this class and date.'))
    ...
    def action_confirm(self):
        ...
        created_attendances = self.env['education.attendance'].create(attendance_records)
    ...
    def _prepare_attendance_vals(self, line):
        ...
        if self.env.context.get('track_time'):
            vals.update({
                'check_in_time': datetime.combine(self.date, datetime.min.time().replace(hour=8)),
                'check_out_time': datetime.combine(self.date, datetime.min.time().replace(hour=17)),
            })
    ...
    def _send_parent_notifications(self, attendances):
        ...
        absent_attendances = attendances.filtered(lambda a: a.state in ['absent', 'late'])
    ...
    def _return_success_action(self, attendances):
        ...
        if self.env.context.get('return_to_attendance_list'):
            ...
            return {
                ...
                'params': {
                    'title': _('Success'),
                    'message': _('Attendance records created successfully for %d students.') % len(attendances),
                    'type': 'success',
                }
            }
...
class BulkAttendanceLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one('education.bulk.attendance.wizard', string='Wizard', required=True, ondelete='cascade')
    ...
    last_attendance = fields.Char(string='Last Attendance', compute='_compute_last_attendance')
